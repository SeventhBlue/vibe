...
namespace vibe_hw {
	...
	void VIBE::reInitPartSamples(cv::UMat& gray, std::vector<SuspFgMask> vt) {
		unsigned int seed = time(NULL);
		cv::RNG rng(seed);

		cv::UMat vt_mask = cv::UMat::zeros(gray.rows, gray.cols, CV_8UC1);
		for (int i = 0; i < vt.size(); i++) {
			vt_mask(vt[i].rect).setTo(255);
		}
		...
		cv::UMat part_4 = cv::UMat::zeros(gray.rows, gray.cols, CV_16SC1);

		for (int i = 0; i < samples.size(); i++) {
			rng.fill(mat_4_part, cv::RNG::UNIFORM, -4, 4, true);
			cv::bitwise_and(mat_4_part, mat_4_part, part_4, vt_mask);

			cv::add(part_4, gray_16s_part, part_4);
			part_4.convertTo(mat_8u_part, CV_8UC1);
			mat_8u_part.convertTo(part_4, CV_16SC1);

			cv::bitwise_and(samples[i], samples[i], sample_part, vt_mask);

			cv::subtract(samples[i], sample_part, samples[i]);
			cv::add(samples[i], part_4, samples[i]);
		}
	}
	...
	void VIBE::updateSamples(cv::UMat& gray) {
		...
		//std::cout << "运行完updateSamples..." << std::endl;
	}
