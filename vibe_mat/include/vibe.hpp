...
namespace vibe_hw
{
	...
	public:
		...
		void initSamples(cv::Mat& gray);                                       // 初始化samples
		void reInitSamples(cv::Mat& gray);                                     // 重新初始化samples
		void reInitPartSamples(cv::Mat& gray, std::vector<cv::Rect> vt);       // samples的局部重新初始化
	...
	private:
		...
		std::vector<cv::Mat> samples;   // 图片的样本集
		void buildNeighborArray(cv::Mat& gray);
		void processFgMask();
		void updateSamples(cv::Mat& gray); // 更新samples
	};
}